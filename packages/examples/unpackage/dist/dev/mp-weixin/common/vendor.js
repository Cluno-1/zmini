...
function initRuntimeSocketService() {
  const hosts = "172.16.10.36,192.168.20.1,192.168.19.1,127.0.0.1";
  const port = "8090";
  const id = "mp-weixin_MTYzaT";
...
const version = "3";
{
  index.__f__("log", "at node_modules/uview-plus/libs/config/config.js:5", `
...
const adapter = (config2) => new Promise((resolve2, reject) => {
  ...
  if (config2.method === "UPLOAD") {
    ...
    requestTask = index.uploadFile({ ..._config, ...otherConfig, ...mergeKeys$1(optionalKeys, config2) });
    ...
    requestTask = index.request({ ..._config, ...mergeKeys$1(optionalKeys, config2) });
  }
  if (config2.getTask) {
  ...
  */
  constructor(arg = {}) {
    if (!isPlainObject(arg)) {
      arg = {};
      index.__f__("warn", "at node_modules/uview-plus/libs/luch-request/core/Request.js:40", "设置全局参数必须接收一个Object");
    }
...
if (index && index.upuiParams) {
  index.__f__("log", "at node_modules/uview-plus/libs/config/props.js:206", "setting uview-plus");
  let temp = index.upuiParams();
  if (temp.httpIns) {
    temp.httpIns(http);
  }
  if (temp.options) {
...
const props$1 = defineMixin({
  props: {
    ...
    value: {
      ...
      default: () => props$2.collapse.value
    },
    // 是否手风琴模式
    accordion: {
      type: Boolean,
      default: () => props$2.collapse.accordion
    },
    // 是否显示外边框
    border: {
      type: Boolean,
      default: () => props$2.collapse.border
    }
  }
});
  ...
  async route(options = {}, params = {}) {
    ...
    if (typeof index.$u.routeIntercept === "function") {
      const isNext = await new Promise((resolve2, reject) => {
        index.$u.routeIntercept(mergeConfig2, resolve2);
      });
...
const props = defineMixin({
  props: {
    color: {
      type: String,
      default: () => props$2.line.color
    },
    // 长度，竖向时表现为高度，横向时表现为长度，可以为百分比，带px单位的值等
    length: {
      type: [String, Number],
      default: () => props$2.line.length
    },
    // 线条方向，col-竖向，row-横向
    direction: {
      type: String,
      default: () => props$2.line.direction
    },
    // 是否显示细边框
    hairline: {
      type: Boolean,
      default: () => props$2.line.hairline
    },
    // 线条与上下左右元素的间距，字符串形式，如"30px"、"20px 30px"
    margin: {
      type: [String, Number],
      default: () => props$2.line.margin
    },
    // 是否虚线，true-虚线，false-实线
    dashed: {
      type: Boolean,
      default: () => props$2.line.dashed
    }
  }
});
